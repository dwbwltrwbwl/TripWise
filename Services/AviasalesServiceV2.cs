...
namespace TripWise.Services
{
    public interface IAviasalesRealService
    {
        ...
        // Эти методы можно сделать optional с default implementation
            ...
            foreach (var flightData in response.Data)
            {
                try
                {
                    var flight = new Flight
                    {
                        ...
                        ArrivalTime = ParseDateTime(flightData.ReturnAt ?? flightData.DepartureAt).AddMinutes(flightData.Duration ?? 120),
